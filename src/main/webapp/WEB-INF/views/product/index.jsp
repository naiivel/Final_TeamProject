										...
										<div class="row g-0">
											<div class="col-sm-10 col-md-9 col-lg-8 col-xl-7 d-flex flex-column justify-content-center">
												...
												<p class="card-text">
													...
													<f:formatDate value="${product.product_departure}" pattern="yyyy-MM-dd" /> ~
													<f:formatDate value="${product.product_arrive}" pattern="yyyy-MM-dd" />
												</p>
											</div>
...
<script>
	...
	const makeHtml = array => {
		...
		for (let product of array) {
			...
			html += '</p>';
			html += '</div>';
			html += '<div class="col-sm-2 col-md-3 col-lg-4 col-xl-5 text-center text-sm-end position-relative">';
			html += '<p class="card-text">잔여좌석:' + product.product_seat + '</p>';
			const seat = product.product_seat;
			if (seat > 10) {
				html += '<button class="btn btn-primary">상세보기</button>';
			} else {
				html += '<button type="button" class="btn btn-danger">상세보기';
				html += '<span class="position-absolute top-0 start-0 translate-middle badge rounded-pill bg-danger">마감 임박!</span></button>';
			}
			html +='</div></div></div>';
		}
		return html;
